...
internal sealed partial class LogPage
{
    ...
    private static readonly string[] chipIds = ["##apsg_log_verbose", "##apsg_log_debug", "##apsg_log_info", "##apsg_log_warning", "##apsg_log_error"];
    ...
    private void DrawToolbar()
    {
        ...
        if (SearchField.Draw("##apsg_log_search", Loc.T(L.Log.SearchHint), ref search, ref searchFocused, searchWidth, height, focusSearch))
        {
            ClearSelection();
        }
        ...
        if (PillButton.Draw("##apsg_log_copy", copyLabel, Styling.AccentArc, PillButton.Emphasis.Filled, FontAwesomeIcon.Copy,
        ...
        if (PillButton.Draw("##apsg_log_clear", clearLabel, clearArmed ? Styling.AccentRose : Styling.TextSecondary,
    ...
    private void DrawFilters()
    {
        ...
        if (source is not null)
        {
            ...
            ImGui.SetCursorScreenPos(new Vector2(x, y));
            if (FilterChip.Draw("##apsg_log_source", label, null, Styling.AccentMagenta, true, height, FontAwesomeIcon.Times, Loc.T(L.Log.SourceChipTooltip)))
            {
                source = null;
                ClearSelection();
            }
