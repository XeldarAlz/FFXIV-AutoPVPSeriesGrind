...
internal sealed partial class LogPage
{
    ...
    private const string InspectorDetailId = "##apsg_log_detail";
    ...
    private void DrawSelectionBar()
    {
        ...
        Paint.Surface(dl, origin, origin + new Vector2(width, height), Styling.CardRounding * scale,
            Styling.WithAlpha(Styling.AccentArc, 0.08f), Styling.WithAlpha(Styling.AccentArc, 0.35f));
        ...
        if (PillButton.Draw("##apsg_log_sel_clear", clearLabel, Styling.TextSecondary, PillButton.Emphasis.Ghost, FontAwesomeIcon.Times,
        ...
        if (PillButton.Draw("##apsg_log_sel_copy", copyLabel, Styling.AccentArc, PillButton.Emphasis.Tinted, FontAwesomeIcon.Copy,
                height: Layout.ActionPillHeight))
        {
            CopyRange(low, high);
        }
    ...
    private void DrawLineCard(int index, float height)
    {
        ...
        if (IconButton.Draw(FontAwesomeIcon.Times, "##apsg_log_inspect_close", buttonSize, Styling.TextDim, Loc.T(L.Log.Close)))
        {
            ClearSelection();
        }

        ImGui.SetCursorScreenPos(new Vector2(closeX - buttonSize - HeaderGap * scale * 0.5f, headerMidY - buttonSize * 0.5f));
        if (IconButton.Draw(FontAwesomeIcon.Copy, "##apsg_log_inspect_copy", buttonSize, Styling.AccentArcSoft, Loc.T(L.Log.CopyLine)))
        {
    ...
    private float DrawShowing(Vector2 origin)
    {
        ...
        var hit = Hit.Area("##apsg_log_reset", resetSize);
        var color = hit.Hovered ? Styling.AccentArcSoft : Styling.AccentArc;
