...
internal sealed partial class LogPage
{
    ...
    private const string ListId = "##apsg_log_list";
    private const string MenuId = "##apsg_log_menu";
    ...
    private void DrawRow(ImDrawListPtr dl, int index, Vector2 rowMin, Vector2 rowSize, Columns columns, bool hovered)
    {
        ...
        if (selected)
        {
            Paint.Fill(dl, rowMin, rowMax, Styling.WithAlpha(Styling.AccentArc, hovered ? 0.22f : 0.16f), rounding);
            Paint.Fill(dl, rowMin, new Vector2(rowMin.X + SelectionBarWidth * scale, rowMax.Y), Styling.AccentArc, rounding);
        }
        else if (hovered)
        {
    ...
    private void DrawJumpPill(float scrollY, float visibleHeight, float innerWidth)
    {
        ...
        if (PillButton.Draw("##apsg_log_jump", label, Styling.AccentArc, PillButton.Emphasis.Filled, FontAwesomeIcon.ArrowDown, height: Layout.ActionPillHeight))
        {
            follow = true;
            newWhileDetached = 0;
        }
    ...
    private void DrawEmpty()
    {
        ...
        if (bufferedCount > 0)
        {
            ...
            if (PillButton.Draw("##apsg_log_reset_empty", Loc.T(L.Log.ResetFilters), Styling.AccentArc, PillButton.Emphasis.Tinted, FontAwesomeIcon.Undo,
        ...
        var footer = Loc.T(L.Log.Footer, ApsgConstants.LogPrefix);
